...
exports.createCliRegistrar = createCliRegistrar;
const fs = __importStar(require("fs/promises"));
const path = __importStar(require("path"));
const os = __importStar(require("os"));
function createCliRegistrar(api) {
    return (ctx) => {
        ...
        cmd.command('setup')
            ...
            try {
                // TRY 1: Use Injected Runtime API (Best Practice)
                if (api && api.runtime && api.runtime.config) {
                    // Load fresh config
                    ...
                    p.note('Saved via Runtime API', 'Success');
                }
                else {
                    // TRY 2: Fallback to Direct File I/O
                    ...
                    p.note(`Saved via File I/O to ${configPath}`, 'Success');
                }
